	...
	if(ObiektXMLHttp) 
	{
		...
		ObiektXMLHttp.onreadystatechange = function()
		{
				if (ObiektXMLHttp.readyState == 4)
				{
					...
					{
						text = text + "<li><a href=\"" + 
						ObiektXMLHttp.responseXML.documentElement.getElementsByTagName("link")[i].childNodes[0].nodeValue +"\" " +
						ObiektXMLHttp.responseXML.documentElement.getElementsByTagName("zdarzenie")[i].childNodes[0].nodeValue +  ">" +
						ObiektXMLHttp.responseXML.documentElement.getElementsByTagName("nazwa")[i].childNodes[0].nodeValue + "</a></li>";
					}
	...
	if(ObiektXMLHttp2) 
	{
		...
		ObiektXMLHttp2.onreadystatechange = function() 
		{
			if (ObiektXMLHttp2.readyState <  4)
				cel2.innerHTML = "<img src='img/wait.gif' style='padding-left:320px;'/>"; //loading
			if (ObiektXMLHttp2.readyState == 4)
			{
				...
				if(numer_wezla < 0) //jesli podano niepoprawną wartosc wezla, to wypisz wszystkie
				{
					...
					//cel.innerHTML = ObiektXMLHttp.responseXML.documentElement.getElementsByTagName("link")[1].childNodes[0].nodeValue; 
						tekst2=tekst2 +  ObiektXMLHttp2.responseXML.documentElement.getElementsByTagName("sekcja")[i].childNodes[0].nodeValue + "<br/>";				tekst3=tekst3 + ObiektXMLHttp2.responseXML.documentElement.getElementsByTagName("tresc")[i].childNodes[0].nodeValue + "<br/>";					}
				}
				
				else
				{
					tekst2=tekst2 +  ObiektXMLHttp2.responseXML.documentElement.getElementsByTagName("sekcja")[numer_wezla].childNodes[0].nodeValue;				tekst3=tekst3 + ObiektXMLHttp2.responseXML.documentElement.getElementsByTagName("tresc")[numer_wezla].childNodes[0].nodeValue;
				}
