...
<html>
	<head>
		...
		<script  src="../skrypty_js/usun_propozycje.js" type="text/javascript"></script>
	...
	<body style="text-align:center;">
		<div class="pojemnik_administracja">
			...
			<div class="tresc_administracja">
				...
				<div class="dane">
					<h1>Usuń propozycję</h1>
					Zaznacz wyłącznie rekordy przeznaczone do usunięcia z bazy danych:<br />
<?php
	include 'polacz.php';
	$sql = "SELECT id,imienazw,telefon,email,marka,rodzajsilnika,przebieg,rokprodukcji,kolor,cena,informacje,zdjecie1, zdjecie2 FROM propozycje";
	$result = mysql_query( $sql, $connection);
	if(! $result ) {
	die('Could not get data: ' . mysql_error());
	}
	echo"<br/><form name='form5' id='form5'><table border='1px'><tr>
		<th>Usuń</th>
	...
	echo "<tr>".
			...
			if($row[zdjecie1]!='')
				echo "<td><input type='button' value='Zdjęcie' onClick=\"window.open('../{$row['zdjecie1']}','Zdjęcie','width=800,height=600,scrollbars=1')\"/></td>";
			else
				echo "<td>brak</td>";
				
			if($row[zdjecie2]!='')
				echo "<td><input type='button' value='Zdjęcie' onClick=\"window.open('../{$row['zdjecie2']}','Zdjęcie','width=800,height=600,scrollbars=1')\"/></td></tr>";
			else
				echo "<td>brak</td></tr>";
	}
	echo "</table><br /><br />
